...
class crud_film extends connDB{
    ...
    public function insertDataFilm($name,$img,$des,$totalE,$length,$year,$country,$updateDay,$stt){
        $sql = "INSERT INTO film SET film_name=:name, img=:img, description=:des, total_episode=:total,length_movies=:lengthM,year=:year,country=:country,update_day=:updateday,status=:stt,total_viewer=0, total_Assess=0";
        $q = $this->conn->prepare($sql);
