<div style="margin:0px 0px 0px 50px;height:500px;width:80%;">
	<?php
        $stm=$pdo->prepare("SELECT comment.id_film,comment.id_user,comment.content,comment.datetime,user.id_user,user.username FROM comment INNER JOIN user ON comment.id_user = user.id_user where id_film=:id_film and comment.state=1 order by id_cmt DESC ");
