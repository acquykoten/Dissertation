$(function(){
	...
	var image_count = $('.slide-image-1').length;
	var image_width = $('.slide-image-1').width();
	...
	$('.slide-holder-1').css("width", display_width + "px");
	
	// Chỉnh lại độ dài của khối bao quanh các slide-image (slide-stage) cho vừa bằng tổng số khối slide-image
	$('.slide-stage-1').css("width", stage_width + "px");
	

	// ========= Xử lý khi click nút next và prev =========
	
	function left_slide_circle(){
		$('.slide-image:last-child').addClass('swapthis-1'); // Xác định hình đang ở vị trí cuối cùng
		$('.swapthis-1').insertBefore('.slide-image-1:first'); // Chuyển hình cuối lên trước hình đầu tiên
		$('.slide-stage-1').css("left",-image_width + "px"); // Dịch chuyển tức thời vị trí của stage sang một khoảng bằng 1 slide-image
		$('.slide-stage-1').stop().animate({left:0},anispeed); // Thực hiện animation
		$('.swapthis-1').removeClass('swapthis-1'); // Reset class cho slide-image vừa chuyển
	}
	function right_slide_circle(){
		$('.slide-image:first').addClass('swapthis-1'); // Xác định hình đang ở vị trí cuối cùng
		$('.swapthis-1').insertAfter('.slide-image-1:last-child'); // Chuyển hình cuối lên trước hình đầu tiên
		$('.slide-stage-1').css("left",-(stage_width - image_width - display_width) + "px"); // Dịch chuyển tức thời vị trí của stage sang một khoảng bằng 1 slide-image
		$('.slide-stage-1').stop().animate({left:display_width - stage_width},anispeed); // Thực hiện animation
		$('.swapthis-1').removeClass('swapthis-1'); // Reset class cho slide-image vừa chuyển
	}
	function left_slide_scroll(){
		// Xác định xem slide có còn hình phía bên trái hay không dựa trên Left của stage
		var stage_left = $('.slide-stage-1').position().left;
		
		// Nếu có
		if(stage_left < 0 && !$('.slide-stage-1').is(':animated')){ //Kích hoạt chỉ khi stage ko chuyển động nữa
		...
		{	
			switch(circle_method){ //Lựa chọn
				case 1:
					$('.slide-stage-1').stop().animate({left:display_width - stage_width},anispeed);
					break;
				case 2:
					left_slide_circle();
					break;
	...
	function right_slide_scroll(){
		...
		var stage_left = Math.abs($('.slide-stage-1').position().left);
		var right_remain = stage_width - (display_width + stage_left);
		
		// Nếu có
		if(right_remain > 0 && !$('.slide-stage-1').is(':animated')){ //Kích hoạt chỉ khi stage ko chuyển động nữa
			$('.slide-stage-1').stop().animate({left:"-=" + image_width},anispeed);
		}
		else // Nếu đã chuyển về hình ảnh đầu tiên thì lựa chọn cách thức quay vòng
		{
			switch(circle_method){ //Lựa chọn
				case 1:
					$('.slide-stage-1').stop().animate({left:0},anispeed);
					break;
				case 2:
					right_slide_circle();
					break;
	...
	$('.slide-control-prev-1').click(function(){left_slide_scroll();});
	$('.slide-control-next-1').click(function(){right_slide_scroll();});
